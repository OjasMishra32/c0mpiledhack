...
def make_action(**kwargs):
    defaults = dict(
        id="a1", type=ActionType.place_in_zone, description="place obj_1 in zone_2",
        object_id="obj_1", target_zone="zone_2", status=ActionStatus.awaiting_verification,
...
def test_weighted_verification(hive_state):
    hive_state.world.objects.append(make_object(confidence=0.85))
    action = make_action()
    ...
    action.evidence.append(Evidence(kind=EvidenceKind.worker_report, confidence=1.0, weight=0.30))
...
def test_host_override_always_verifies(hive_state):
    ...
    action.evidence.append(Evidence(kind=EvidenceKind.host_override, confidence=1.0, weight=1.0))
    result = verifier.evaluate(action, hive_state)
...
def test_predicate_object_in_zone_true(hive_state):
    hive_state.world.objects.append(make_object(zone="zone_2"))
    pred = Predicate(type=PredicateType.object_in_zone, subject="obj_1", object="zone_2")
    assert verifier.check_predicate(pred, hive_state) is not None


def test_predicate_object_in_zone_false(hive_state):
    hive_state.world.objects.append(make_object(zone="zone_1"))
    pred = Predicate(type=PredicateType.object_in_zone, subject="obj_1", object="zone_2")
    assert verifier.check_predicate(pred, hive_state) is None


def test_predicate_object_in_zone_missing_object(hive_state):
    pred = Predicate(type=PredicateType.object_in_zone, subject="obj_ghost", object="zone_2")
    assert verifier.check_predicate(pred, hive_state) is None


def test_simulation_evidence_clears_bar_alone(hive_state):
    hive_state.world.objects.append(make_object(confidence=0.95, source="simulation"))
    action = make_action()
    result = verifier.evaluate(action, hive_state)
    assert result.verified is True
...
def test_narrate_mentions_each_source(hive_state):
    evidence = [
        Evidence(kind=EvidenceKind.vision, confidence=0.84, weight=0.60),
        Evidence(kind=EvidenceKind.worker_report, confidence=1.0, weight=0.30),
    ]
