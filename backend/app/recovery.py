...
not built yet) would only ever improve it, never gate it. The critical design
decision throughout is **isolate, don't restart**: pause only the dependency chain
touched by a deviation, and let everything else keep running.
...
def detect_wrong_object_moved(state) -> list[DeviationTrigger]:
    out = []
    active_object_ids = {
        a.object_id for a in state.actions.values()
        if a.status in (ActionStatus.dispatched, ActionStatus.executing, ActionStatus.awaiting_verification)
    }
...
def detect_left_target_zone(state) -> list[DeviationTrigger]:
    ...
    for a in state.actions.values():
        ...
        if not a.target_zone or not a.object_id:
            continue
        obj = next((o for o in state.world.objects if o.id == a.object_id), None)
...
def detect_worker_disconnected(state) -> list[DeviationTrigger]:
    ...
    for w in state.workers.values():
        if w.connected or not w.current_action_id:
            continue
        try:
            last_seen = datetime.fromisoformat(w.last_seen_at)
        except ValueError:
            last_seen = now
        if (now - last_seen).total_seconds() > 8:
...
def detect_worker_blocked(state) -> list[DeviationTrigger]:
    return [
        ...
        for a in state.actions.values() if a.status == ActionStatus.blocked
    ]
...
def detect_worker_paused(state) -> list[DeviationTrigger]:
    return [
        ...
        for w in state.workers.values() if w.status == WorkerStatus.paused and w.current_action_id
    ]


def detect_conflicting_manipulation(state) -> list[DeviationTrigger]:
    out = []
    active = [a for a in state.actions.values()
              if a.status in (ActionStatus.dispatched, ActionStatus.executing) and a.object_id]
    seen: set[str] = set()
...
def detect_scheduler_deadlock(state) -> list[DeviationTrigger]:
    if state.execution_status != "executing":
        return []
    live = any(a.status in (
        ActionStatus.available, ActionStatus.assigned, ActionStatus.dispatched,
        ActionStatus.executing, ActionStatus.awaiting_verification,
    ) for a in state.actions.values())
    stuck = any(a.status in (ActionStatus.queued, ActionStatus.blocked) for a in state.actions.values())
    if not live and stuck:
        return [DeviationTrigger(kind="scheduler_deadlock", detail="empty batch with actions still unresolved")]
    return []
    ...
    for t in candidates:
        ...
        if n >= threshold:
            ...
            if t.kind == "wrong_object_moved" and t.object_id:
                obj = next((o for o in state.world.objects if o.id == t.object_id), None)
                if obj:
...
def retrieve_and_restore(trigger: DeviationTrigger, state) -> RecoveryPlan:
    ...
    pause_ids = [a.id for a in dependents if a.status not in (ActionStatus.verified, ActionStatus.cancelled)]
    released = [lock for lock in state.locks if trigger.object_id and trigger.object_id in lock]
    obj = next((o for o in state.world.objects if o.id == trigger.object_id), None)
...
def reissue_with_correction(trigger: DeviationTrigger, state) -> RecoveryPlan:
    obj = next((o for o in state.world.objects if o.id == trigger.object_id), None)
    return RecoveryPlan(
        reassign={trigger.action_id: state.actions[trigger.action_id].assigned_worker_id} if trigger.action_id in state.actions else {},
        narration=NARRATION["left_target_zone"].format(
            object=obj.display_label() if obj else trigger.object_id,
            zone=state.actions[trigger.action_id].target_zone if trigger.action_id in state.actions else "?",
        ),
        confidence=0.7,
    )
...
def reverify_then_correct(trigger: DeviationTrigger, state) -> RecoveryPlan:
    a = state.actions.get(trigger.action_id)
    obj = next((o for o in state.world.objects if o.id == trigger.object_id), None)
    return RecoveryPlan(
        reassign={trigger.action_id: a.assigned_worker_id} if a else {},
...
def retry_then_reassign(trigger: DeviationTrigger, state) -> RecoveryPlan:
    a = state.actions.get(trigger.action_id)
    w = state.workers.get(trigger.worker_id) if trigger.worker_id else None
...
def reassign(trigger: DeviationTrigger, state) -> RecoveryPlan:
    a = state.actions.get(trigger.action_id) if trigger.action_id else None
    w = state.workers.get(trigger.worker_id) if trigger.worker_id else None
...
def freeze_and_serialize(trigger: DeviationTrigger, state) -> RecoveryPlan:
    a = state.actions.get(trigger.action_id) if trigger.action_id else None
    obj = next((o for o in state.world.objects if o.id == trigger.object_id), None)
    pause_ids = [a.id] if a and a.priority == min(
        (x.priority for x in state.actions.values() if x.object_id == trigger.object_id), default=0
    ) else []
    return RecoveryPlan(
        pause_action_ids=pause_ids,
...
def rebuild_remaining_graph(trigger: DeviationTrigger, state) -> RecoveryPlan:
    stuck_ids = [a.id for a in state.actions.values() if a.status in (ActionStatus.queued, ActionStatus.blocked)]
    return RecoveryPlan(
...
def _dependents_of_object(object_id: str | None, state) -> list[Action]:
    if not object_id:
        return []
    roots = [a.id for a in state.actions.values() if a.object_id == object_id or a.target_object_id == object_id]
    out: list[Action] = []
    for r in roots:
...
async def apply_recovery_plan(plan: RecoveryPlan, state) -> None:
    ...
    for aid in plan.pause_action_ids:
        a = state.actions.get(aid)
        if a and a.status not in (ActionStatus.verified, ActionStatus.cancelled):
    ...
    if plan.narration:
        await state.emit("recovery_started", plan.narration, severity="warn", confidence=plan.confidence)
